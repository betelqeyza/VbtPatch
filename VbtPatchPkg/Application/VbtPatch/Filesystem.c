    ...
    IN VOID (*WaitForEnterCallback)(BOOLEAN))
{
  ...
  if ((FilePath == NULL) || (mVbtPatchImageInfo == NULL) || (mVbtPatchImage == NULL))
  {
    return EFI_INVALID_PARAMETER;
  }
  ...
  //
  FilePathOnDevice = FileDevicePath(mVbtPatchImageInfo->DeviceHandle, FilePath);
  FilePathOnDeviceText = ConvertDevicePathToText(FilePathOnDevice, TRUE, FALSE);
  Status = gBS->LoadImage(FALSE, mVbtPatchImage, FilePathOnDevice, NULL, 0, &FileImageHandle);
  ...
  CHAR16 *ParentDeviceText = NULL;
  CHAR16 *FileImageDeviceText = NULL;
  EFI_HANDLE parentHandle = mVbtPatchImageInfo->DeviceHandle;
  ParentDeviceText = ConvertDevicePathToText(DevicePathFromHandle(parentHandle), TRUE, FALSE);
  FileImageDeviceText = ConvertDevicePathToText(DevicePathFromHandle(FileImageInfo->DeviceHandle), TRUE, FALSE);

  PrintDebug(L"Parent Device Handle -> %p, path: %s\n", parentHandle, ParentDeviceText ? ParentDeviceText : L"<null>");
  PrintDebug(L"Loaded Image Device Handle -> %p, path: %s\n", FileImageInfo->DeviceHandle, FileImageDeviceText ? FileImageDeviceText : L"<null>");

  if (ParentDeviceText)
    FreePool(ParentDeviceText);
  if (FileImageDeviceText)
    FreePool(FileImageDeviceText);
